...
describe("DataLakePathClient", () => {
  ...
  it("DataLakeFileClient create with default parameters", async () => {
  ...
  it("DataLakeFileClient create with relative expiry", async () => {
    ...
    await testFileClient.create({ timeToExpireInMs: timeToExpireInMs });
    const result = await testFileClient.getProperties();
  ...
  it("DataLakeFileClient create with relative and absolute expiry", async () => {
    const now = new Date();
    const delta = 20 * 1000;
    const expiresOn = new Date(now.getTime() + delta);
    const timeToExpireInMs = 60 * 60 * 1000; // 1hour
    ...
    const testFileClient = fileSystemClient.getFileClient(testFileName);
    try {
      await testFileClient.create({ expiresOn: expiresOn, timeToExpireInMs: timeToExpireInMs });
    } catch (error) {
      assert.equal((error as any).message, "timeToExpireInMs and expiresOn cannot both be set.");
    }
  });
  ...
  it("DataLakeFileClient createIfNotExists with relative expiry", async () => {
    ...
    const testFileClient = fileSystemClient.getFileClient(testFileName);
    const timeToExpireInMs = 60 * 60 * 1000; // 1hour
    await testFileClient.createIfNotExists({ timeToExpireInMs: timeToExpireInMs });
    const result = await testFileClient.getProperties();
    assert.equal(result.createdOn!.getTime() + 1000 * 3600, result.expiresOn!.getTime());
  });
  ...
  it("DataLakeFileClient createIfNotExists with relative and absolute expiry", async () => {
    const now = new Date();
    const delta = 20 * 1000;
    const expiresOn = new Date(now.getTime() + delta);
    const timeToExpireInMs = 60 * 60 * 1000; // 1hour
    ...
    try {
      await testFileClient.createIfNotExists({
        expiresOn: expiresOn,
        timeToExpireInMs: timeToExpireInMs,
      });
    } catch (error) {
      assert.equal((error as any).message, "timeToExpireInMs and expiresOn cannot both be set.");
    }
  ...
  it("DataLakeDirectoryClient create with relative expiry", async () => {
    ...
    const timeToExpireInMs = 60 * 60 * 1000; // 1hour
    try {
      await testDirClient.create({ timeToExpireInMs: timeToExpireInMs });
  ...
  it("DataLakeDirectoryClient createIfNotExists with relative expiry", async () => {
    ...
    const timeToExpireInMs = 60 * 60 * 1000; // 1hour
    try {
      await testDirClient.createIfNotExists({ timeToExpireInMs: timeToExpireInMs });
      assert.fail("Creating directory with expiry should fail.");
    } catch (error) {
      assert.ok((error as any).message.includes("Set Expiry is not supported for a directory"));
    }
